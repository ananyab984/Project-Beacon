...
import { LayoutGrid, ContactRound, Mail, MessagesSquare, LineChart, Plus, LogOut } from "lucide-react";
...
const nav: NavItem[] = [
  ...
  { to: "/recruiter/performance", label: "Lead Performance", icon: LineChart },
];

function RecruiterLayout() {
  const path = useRouterState({ select: (s) => s.location.pathname });
  const current = nav.find((n) => (n.to === "/recruiter" ? path === "/recruiter" || path === "/recruiter/" : path.startsWith(n.to)));
    ...
    <div className="min-h-screen bg-background text-foreground" style={{ fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif" }}>
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-60 flex-col border-r border-sidebar-border bg-sidebar text-sidebar-foreground md:flex">
        ...
        <nav className="mt-2 flex flex-1 flex-col gap-0.5 px-3">
          {nav.map((item) => {
            const isActive = item.to === "/recruiter" ? path === "/recruiter" || path === "/recruiter/" : path.startsWith(item.to);
          ...
          <DropdownMenuTrigger asChild>
            <button className="mx-3 my-3 flex items-center gap-2 rounded-lg border border-sidebar-border bg-sidebar-accent p-3 text-left transition-colors hover:border-primary/60 hover:bg-sidebar-accent/90">
              ...
              <div className="min-w-0 flex-1">
                <div className="truncate text-sm font-medium text-sidebar-primary-foreground">{user?.name ?? "Recruiter"}</div>
                <div className="truncate text-[11px] font-medium text-sidebar-foreground capitalize">{user?.role ?? "recruiter"}</div>
              </div>
            </button>
          </DropdownMenuTrigger>
